...
var $scIsIE = false;
if (navigator.userAgent.indexOf("MSIE") != -1){ $scIsIE = true }
...
boa.common = (function() {
    ...
    function dialogSetup() {
        $('body').on('click', '.boa-dialog', function(ev) {
    ...
    function actionLayerSetup() {
        $('body').on('click', '.boa-action', function(ev) {
            var $link = $(this),
            ...
            $modal.dialog({
                ...
                open: function() {
                    ...
                    if ($modalCloseLink.find('.bac-dialog-open').length === 0) {
                        ...
                        var endOfLayerADAText = $boaLangObj.endOfLayer;
                        /*
                        * The newer jQuery-ui versions (> 1.11.0) append a <button> for ui-close-icon instead of <a> element 
                        * ADA defect fix for screenreader not reading the "close" content, since the "close" text is not wrapped in a <span> element and let loose
                        * Fix for focus issues, because of the height of the button
                        * Making the button border to none, since the browsers user agent styles are adding a border-width.
                        */
    ...
    var $this, tempShim, tempPosition, tempWidth, tempData, shimHolder, //setup reusable vars
        ...
        init = function() { //initialize data and listeners on each element
            var $selects = $(selector),
                runThis = window.jQuery.fn.jquery === "3.3.1" ? ($selects.length && $scIsIE && $IEver < 9) : ($selects.length && $.browser.msie && $.browser.version < 9), //save conditional killswitch (IE8- and select-bofa elements on page)
...
var cfLoader = (function($) {
    ...
    var init = function() { //publicly reachable API
        ...
        conductor = function() { //ORCHESTRATES AND STARTS ALL THE GOOD STUFF
            ...
            typeof jQuery.fn.jquery !== 'undefined' && jQuery.fn.jquery == "1.3.2") return $('html').addClass('cf-force-show-crowbar'); //if dependency library not available, force show and stop running.
                ...
                "browserSupport": {
                    ...
                    "rules": function(b) {
                        ...
                        /*Unsupported browser types: Not big 4 (Win IE, Chrome, Win FF, Mac Safari)*/
                            ...
                            (b.msie && b.version <= 10) || //msie
                            (b.chrome && b.version <= 55) || //chrome 
                            (b.firefox && b.version <= 50) || //firefox
                            (b.safari && b.version <= 9)) { //safari
